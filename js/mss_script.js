    ...
    jQuery.ajax({
        ...
        success: function(data) {
            if (data.indexOf("success") != -1) {
                ...
                document.getElementById('msst-vtiger-test-results').innerHTML = "Vtiger Credentials are wrong";
            }
        },
...
function move(position, direction, siteurl, module, option, onAction) {
    ...
    var shortcode = jQuery('#mss_label' + new_position).val();
    if (onAction == 'onEditShortCode') {
        // shortcode = jQuery('#mss_label'+position+1).val();
    }
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
...
function recature() {
    var captueval = jQuery('#wp_tiger_msst_user_capture').attr("checked");
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        data: {
...
function doAction(shortcode, module, adminurl) {
    ...
    if (selected_action == 'edit') {
        ...
        link_url = adminurl + "admin.php?page=Mss_tiger&action=" + action;
