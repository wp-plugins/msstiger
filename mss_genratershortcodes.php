...
add_shortcode('display_contact_page', array('MSstWPTigerShortcodes', 'msst_display_page'));

add_shortcode('display_widget_area', array('MSstWPTigerShortcodes', 'msst_display_widget'));

add_shortcode('Mss-tiger-New-form name=', array('MSstWPTigerShortcodes', 'msst_genrate_code'));

class MSstWPTigerShortcodes {
	public static function msst_display_page($atts) {
		...
		if (isset($_REQUEST['page_contactform'])) {
			...
			$successfulAtemptsOption = get_option("mss_tiger-contact-form-attempts");
	...
	public static function msst_display_widget($atts) {
		...
		if (isset($_REQUEST['widget_contactform'])) {
			...
			$successfulAtemptsOption = get_option("mss-tiger-contact-widget-form-attempts");
		...
		if (is_array($config_widget_field['widgetfieldlist'])) {
			foreach ($selectedFields as $field) {
				...
				//			$content1.="<input type='text' class='MSs-tiger-widget-area-text' size='20' value='' name='".$field->fieldname."' id='".$field->fieldname."'></p>";
				$content1 .= "</td></tr>";
				$content .= $content1;
			}
		}
		$content .= "<tr><td></td><td>";
		$content .= "<p>";
		$content .= "<input type='submit' class='MSs-tiger-widget-area-submit' value='Submit' id='submit' name='submit'></p></td></tr>";
		$content .= "</table>";
	...
	public static function msst_genrate_code($atts) {
		...
		for ($i = 0; $i <= count($resut); $i++) {
			...
			if (isset($resut["msst_vtlc_field$i"])) {
				array_push($fieldArr, $resut["msst_vtlc_field_hidden$i"]);
			}
		}
