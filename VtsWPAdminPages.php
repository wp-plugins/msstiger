...
global $wpdb;
	...
	<?php
			...
			echo "<div style='margin-top:20px;font-weight:bold;'>
					Please enter a valid database <a href=" . admin_url() . "admin.php?page=Vts_tiger&action=plugin_settings>settings</a>
					</div>";
		}
	}
	...
	function capture_wp_users() {
		global $plugin_url_vts_tiger;
		$imagepath = "{$plugin_url_vts_tiger}/images/";
...
if ($config['vtst_user_capture'] == 'on') {
	...
	function plugin_settings() {
		...
		$fieldNames = array('hostname' => __('Database Host'), 'dbuser' => __('Database User'), 'dbpass' => __('Database Password'), 'dbname' => __('Database Name'), 'url' => __('URL'), 'Vts_host_username' => __("MSst Host Username"), 'VTS_host_access_key' => __("MSst Host Access Key"), 'wp_tiger_vtst_user_capture' => __('Capture User'));
	...
	function widget_fields() {
		...
		if (($_REQUEST['action'] = 'widget_fields') && (empty($_REQUEST['EditShortCode']))) {
			...
			if (isset($_POST['saveContactFields']) && $_POST['saveContactFields'] == 'Save Field Settings') {
				...
				$linkUrl = site_url() . '/wp-admin/admin.php?page=Vts_tiger&action=widget_fields';
				?>
				<script>
...
<br/>
			...
			foreach ($allowedFields as $key => $field) {
				...
				if (in_array($field->fieldid, $config_widget_field['widgetfieldlist'])) {
					if ($typeofdata[1] == 'M') {
						...
						$widgetContent .= '<a id="publish3" class="smack_pointer" onclick="published(' . $key . ',&quot;0&quot;,&quot;' . admin_url() . '&quot;,&quot;widget&quot;,&quot;' . $field->fieldid . '&quot;,&quot;onCreate&quot;);" name="publish3"><img  id="vtst-field-td-middleit" src="' . $imagepath . 'tick.png"  /></a>';
					}
				} else {
					$widgetContent .= '<a id="publish3" class="smack_pointer" onclick="published(' . $key . ',&quot;1&quot;,&quot;' . admin_url() . '&quot;,&quot;widget&quot;,&quot;' . $field->fieldid . '&quot;,&quot;onCreate&quot;);" name="publish3"><img  id="vtst-field-td-middleit" src="' . $imagepath . 'publish_x.png"  /></a>';
				}
	...
	function vtiger_db_fields() {
		...
		if (($_REQUEST['action'] = 'vtiger_db_fields') && (empty($_REQUEST['EditShortCode']))) {
			if (isset($_POST['field_posted'])) {
				...
				update_option('Vts_vtpl_field_settings', $config_field);
			}
		}
		$content = '';
		...
		if (!empty($allowedFields)) {
			...
			if (($_REQUEST['action'] = 'vtiger_db_fields') && (empty($_REQUEST['EditShortCode']))) {
				...
				if (isset($_POST['Submit']) && $_POST['Submit'] == 'Save Field Settings') {
					...
					$update = update_option('Vts_vtlc_widget_field_settings', $config_widget_field);
					$linkUrl = site_url() . '/wp-admin/admin.php?page=Vts_tiger&action=vtiger_db_fields';
					?>
				<script>
					 saveFormFields('<?php echo $data_array;?>','<?php echo $linkUrl;?>');
				</script>
		...
		<div id="fieldtable">
			...
			foreach ($allowedFields as $key => $field) {
				...
				$content .= '<td class="vtst-field-td-middleit"><input type="hidden"
							...
							value="' . $typeofdata[1] . '"
							id="field_type' . $key . '"> <input type="hidden" id="vts_label' . $key . '"
				...
				if ($typeofdata[1] == 'M') {
					$content .= '<input type="hidden"
						value="' . $field->fieldname . '"
							...
							name="vtst_vtlc_field' . $key . '" /> <input
				...
				if (in_array($field->fieldid, $config_field['fieldlist'])) {
					...
					if ($typeofdata[1] == 'M') {
						...
						$content .= '<a id="publish3" class="smack_pointer" onclick="published(' . $key . ',&quot;0&quot;,&quot;' . admin_url() . '&quot;,&quot;lead&quot;,&quot;' . $field->fieldid . '&quot;,&quot;onCreate&quot;);" name="publish3"><img  id="vtst-field-td-middleit"src="' . $imagepath . 'tick.png" /></a>';
					}
				} else {
					$content .= '<a id="publish3" class="smack_pointer" onclick="published(' . $key . ',&quot;1&quot;,&quot;' . admin_url() . '&quot;,&quot;lead&quot;,&quot;' . $field->fieldid . '&quot;,&quot;onCreate&quot;);" name="publish3"><img  id="vtst-field-td-middleit" src="' . $imagepath . 'publish_x.png" /></a>';
				}
				$content .= '</td>
					<td class="vtst-field-td-middleit">';
				if ($inc == 1) {
	...
	function deleteShortCodes() {
		...
		if ($_REQUEST['action'] == 'deleteShortCodes') {
			...
			if ($delte_reqst) {?>
	<script>window.location.href ="admin.php?page=Vts_tiger&action=vts_tiger_listShortcodes";</script>;
	<?php
}
		}
	}
